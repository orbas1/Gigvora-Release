...
@php
    $feedCardPresenter = app(\App\Support\Feed\FeedCardPresenter::class);
    $inlineAdInserted = false;
...
@foreach ($posts as $loopIndex => $post)
    ...
    @endphp
    @php
        $feedMeta = $feedCardPresenter->present($post);
    @endphp
    @if(!$inlineAdInserted && !empty($inlineAd) && $loopIndex === 2)
        <div class="gv-card gv-feed-card gv-feed-inline-ad">
            @include('advertisement::components.ad_feed_card', ['ad' => $inlineAd])
        </div>
        @php $inlineAdInserted = true; @endphp
    @endif
    ...
    <div class="single-item-countable single-entry" id="postIdentification{{ $post->post_id }}">
        <div class="entry-inner">
            ...
            @if(!empty($feedMeta['eyebrow']))
                <div class="gv-feed-eyebrow mb-2">
                    <i class="{{ $feedMeta['icon'] }}"></i>
                    <span>{{ $feedMeta['eyebrow'] }}</span>
                </div>
            @endif
                    ...
                    <div class="ava-desc ms-2">
                        <h3 class="mb-0">
                            ...
                            @else
                              ...
                              $badge = \App\Models\Badge::where('user_id', $post->user_id)->first();
                              $badgeActive = $badge && $badge->isActive();
                              @endphp
                              <div class="eUserFeature">
            ...
            <div class="e_comment">
                ...
                @if (!empty($feedMeta['cta_label']) && !empty($feedMeta['cta_url']))
                    <div class="gv-feed-item-cta">
                        <a href="{{ $feedMeta['cta_url'] }}">
                            {{ $feedMeta['cta_label'] }} <i class="fa-solid fa-arrow-up-right-from-square"></i>
                        </a>
                    </div>
                @endif
            </div>
