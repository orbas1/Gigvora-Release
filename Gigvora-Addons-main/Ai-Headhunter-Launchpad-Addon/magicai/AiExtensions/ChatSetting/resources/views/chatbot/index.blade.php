...
@section('titlebar_actions')
    <x-button
            ...
            href="{{ $app_is_demo ? '#' : LaravelLocalization::localizeUrl(route('dashboard.user.chat-setting.chatbot.create')) }}"
    >
        {{ __('Add Template') }}
    </x-button>
@endsection
            ...
            <x-slot:body>
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td>
                            @include('chat-setting::chatbot.status', ['status' => $item->status])
                        </td>
                        <td>
                            <p class="m-0">
                                {{--                                {{ $item->role }}--}}
                        ...
                        <td class="text-end">
                            <x-button
                                    ...
                                    size="none"
                                    href="{{ route('dashboard.user.chat-setting.chatbot.show', $item->id) }}"
                            ...
                            {{--                                variant="ghost-shadow"--}}
                            {{--                                size="none"--}}
                            {{--                                href="{{ route('dashboard.admin.chatbot.edit', $item->id) }}"--}}
                            {{--                                title="{{ __('Edit') }}"--}}
                            {{--                            >--}}
                            ...
                            <x-button
                                    ...
                                    size="none"
                                    onclick="deleteChatbot('{{ route('dashboard.user.chat-setting.chatbot.destroy', $item->id) }}')"
                                    title="{{ __('Delete') }}"
                            >
