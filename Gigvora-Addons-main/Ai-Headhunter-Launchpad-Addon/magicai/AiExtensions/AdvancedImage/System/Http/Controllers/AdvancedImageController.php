...
class AdvancedImageController extends Controller
{
    ...
    {
        $task = UserOpenai::query()->findOrFail($id);

        if ($task['status'] === 'CREATED' || $task['status'] === 'IN_PROGRESS') {
            $model = data_get($task['payload'], 'model');

            if ($model === 'freepik') {
                $task = $this->freepikService->checkStatus($task);
            }
            if ($model === 'novita') {
                $task = $this->novitaService->checkStatus($task);
            }
        }
    ...
    public function webhook(Request $request, string $model = 'freepik'): void
    {

        if ($model === 'freepik') {
            $taskId = $request->get('request_id');

            $task = UserOpenai::query()->where('request_id', $taskId)->firstOrFail();

            $this->freepikService->webhook($task, $request->all());
        }
        if ($model === 'novita') {
            $taskId = $request->input('payload.task.task_id');
    ...
    public function editor(Request $request)
    {
        ...
        $driver = Entity::driver(EntityEnum::tryFrom($request->get('ai_model')))->inputImageCount(1)->calculateCredit();
        ...
        if ($request->get('ai_model') === 'freepik') {
        ...
        if ($request->get('ai_model') === 'clipdrop') {
        ...
        if ($request->get('ai_model') === 'novita') {
        ...
        if (is_string($photo) || ($taskId && ($taskStatus === 'IN_PROGRESS' || $taskStatus === 'CREATED'))) {
