...
- **Icons & styling:** Use the host icon set (e.g., Font Awesome) and Gigvora button/card classes; the dashboard view pulls its scripts through `@vite` to stay on-brand with the host build pipeline. Vite entrypoints include `resources/js/advertisement/dashboard.js`, with matching placeholders ready for campaigns, creatives, reports, keyword planner, forecast, and admin screens.
