<script>
    function sendMidjourneyGeneratorForm(ev) {
        ev?.preventDefault();
        ev?.stopPropagation();

        const submitBtn = document.getElementById("midjourney_generator_button");

        document.getElementById("midjourney_generator_button").disabled = true;
        document.getElementById("midjourney_generator_button").innerHTML = magicai_localize.please_wait;
        ...
        @if ($openai->type === 'image')
        	let imageGenerator = document.querySelector('[data-generator-name][data-active=true]')?.getAttribute('data-generator-name');
        @endif
        ...
        formData.append('description', $("#description_midjourney").val());
        formData.append('model', 'midjourney');
        formData.append('image_number_of_images', 1);
        formData.append('image_generator', '{{ \App\Domains\Entity\Enums\EntityEnum::MIDJOURNEY->value }}');
        ...
        formData.append('description_midjourney', $('#description_midjourney').val());
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.status !== 'success' && (res.message)) {
					document.getElementById("midjourney_generator_button").disabled = false;
					document.getElementById("midjourney_generator_button").innerHTML = "Regenerate";
...
//show successful message
    ...
    document.addEventListener('DOMContentLoaded', function () {
        setInterval(checkImageStatusMidjourney, 5000);
    });
</script>
