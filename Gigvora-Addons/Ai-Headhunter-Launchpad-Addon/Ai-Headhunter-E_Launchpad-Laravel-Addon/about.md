...
- **Navigation placement:** A top-level **Talent & AI** dropdown (with icons that mirror the Flutter app) links to **Headhunters**, **Experience Launchpad**, **AI Workspace**, **Volunteering**, and an admin-only configuration panel. Visibility is tied to `gigvora_talent_ai.enabled`, each `gigvora_talent_ai.modules.*.enabled` flag, and the `manage_talent_ai` gate for admin entries.
- **Key screens & routes:**
...
- **Styling:** The shared stylesheet `mix('css/addons/talent_ai/talent_ai.css')` scopes cards, pipelines, AI workspaces, and volunteering tiles inside `.talent-ai-page`, ensuring consistent typography and spacing with Sociopro’s Tailwind base while preventing global overrides.

## Mobile Integration
- **Dependencies:** `talent_ai_flutter_addon` added as a local dependency in the Flutter shell (`../Gigvora-Addons/Ai-Headhunter-Launchpad-Addon/Ai-Headhunter-E_Launchpad-flutter_addon`).
...
- **Seeders:** `Database\Seeders\TalentAiSeeder` provisions AI subscription plans from `config/gigvora_talent_ai.php` and is invoked automatically from the host `DatabaseSeeder`. Seed data only inserts configurable plans, so it is safe to rerun in production.
- **Ordering & uniqueness:** migration filenames start with `2024_01_01_*` so they execute after the Sociopro core schema. Unique constraints exist on mandate/candidate pairs, BYOK credentials per user/provider, and AI subscription rows to prevent duplicate data when syncing environments.
