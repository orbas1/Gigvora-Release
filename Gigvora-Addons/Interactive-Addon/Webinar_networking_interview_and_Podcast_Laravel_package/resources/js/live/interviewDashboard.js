...
if (candidateDetail) {
    ...
    const refreshJoinState = () => {
        ...
        if (!start) {
            updateButtonState(joinInterview, true);
            joinInterview.textContent = joinInterview.dataset.readyLabel || joinInterview.textContent;
            return;
        }
...
if (waitingRootInterview) {
    ...
    const tick = () => {
        if (!countdown || !enterBtn) return;

        if (!start) {
            countdown.textContent = '--:--';
            updateButtonState(enterBtn, true);
            if (waitingStatus) {
