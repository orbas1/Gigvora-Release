...
<div class="space-y-6">
    ...
    <form method="get" class="gv-card space-y-4">
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="space-y-1">
                <span class="gv-label">{{ get_phrase('My webinars') }}</span>
                <label class="inline-flex items-center gap-2 text-sm text-[var(--gv-color-neutral-600)]">
                    <input type="checkbox"
                        class="h-4 w-4 rounded border-[var(--gv-color-border)] text-[var(--gv-color-primary-600)]"
                        name="mine" value="1" @checked($filters['mine'] ?? false)>
                    <span>{{ get_phrase('Only show webinars I host') }}</span>
                </label>
            </div>
            <div class="space-y-1">
            ...
            <label class="space-y-1">
                <span class="gv-label">{{ get_phrase('Starts after') }}</span>
                <input type="date" name="starts_after" class="gv-input" value="{{ $filters['starts_after'] ?? '' }}">
            </label>
            <label class="space-y-1">
                <span class="gv-label">{{ get_phrase('Starts before') }}</span>
                <input type="date" name="starts_before" class="gv-input" value="{{ $filters['starts_before'] ?? '' }}">
            </label>
        </div>
