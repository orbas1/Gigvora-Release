...
class GigCreation extends Component
{
    ...
    public function removePlan( $key ){

        if(isset($this->gig_plans[$key])){ 
            unset($this->gig_plans[$key]);
        }
    }
    ...
    public function edit($id, $seller_addons){
        ...
        if(!empty($gig->attachments['video_url'])){
            $this->video_url = $gig->attachments['video_url'];
        }
        if(!empty($gig->attachments['files'])){
            foreach($gig->attachments['files'] as $key => $single){
                $this->galleryExistingFiles[$key] = (object) $single;
            }
        }
            ...
            if( !empty($this->galleryExistingFiles) ){
                ...
                foreach($this->galleryExistingFiles as $key => $single){
                    ...
                    if(method_exists($file,'getClientOriginalName')){
                        $file_path = $file->store('gigs/' . $gig->id, 'public');
