...
| Interactive / Live | Live hub, webinars, networking, podcasts, and interviews now use `wnip::layouts.live` + `.gv-card`/`.gv-pill` tokens, quick tools, and shared components (`event_card`, `waiting_room_header`, `live_chat_panel`, `notes_sidebar`, `calendar_widget`). Waiting rooms + live shells share timers, Utilities CTAs, advertisement slots, and interview CTA gating wired to `wnip.interviews.live`. | Monitor for additional admin view parity + future animations, but primary surfaces are tokenized and interview scoring now auto-saves via the scoring panel. |
