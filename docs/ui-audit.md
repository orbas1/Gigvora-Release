...
- **Permission-aware nav & analytics (Task 20)**: Navigation filtering now uses the shared permission matrix (`config/permission_matrix.php`) through `NavigationBuilder`, so Ads/Talent & AI/Admin/Moderation entries disappear for unauthorised roles. API responses capture `analytics.navigation.rendered` while Freelance dashboards/role switches/favourites emit `analytics.freelance.*` events to keep Utilities/Jobs/Freelance telemetry aligned with the taxonomy documented in `logic_flows.md#1.0`.
