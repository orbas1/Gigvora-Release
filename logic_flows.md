...
- **Role matrix**: `config/permission_matrix.php` centralizes platform personas (`member`, `freelancer`, `recruiter`, `company_admin`, `creator`, `moderator`, `platform_admin`) with addon-scoped permission slugs (Jobs `jobs.*`, Freelance `freelance.*`, Ads `ads.*`, Talent & AI `talent_ai.*`, Interactive `interactive.*`, Utilities `utilities.*`, AI `ai.*`, platform `access_admin_panel`/`manage_system_settings`/`viewAnalytics`/`viewSecurity`/`moderate`). `App\Support\Authorization\PermissionMatrix` resolves the active role (from `users.user_role` or `getUserRole()`), registers gates in `AuthServiceProvider`, powers the `permission` middleware, and is documented in `docs/roles_permissions.md`.
